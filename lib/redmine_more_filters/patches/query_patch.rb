    ...
    module QueryPatch
      def self.included(base)
        ...
        base.class_eval do
          ...
          operators_by_filter_type[:string].insert(1, '*=', '!*=', '^=', '*^=', '!^=', '!*^=', '$=', '*$=', '!$=',
                                                   '!*$=', '*~', '!*~', '[~]', '![~]')
          operators_by_filter_type[:text].insert(1, '^=', '*^=', '!^=', '!*^=', '$=', '*$=', '!$=', '!*$=', '*~',
                                                 '!*~', '[~]', '![~]', 'match', '!match')

          operators_by_filter_type[:date].insert(14, 'nm')
          operators_by_filter_type[:date].insert(11, 'nw')
          operators_by_filter_type[:date].insert(9, 'nd')
          operators_by_filter_type[:date].insert(9, '>>')
          operators_by_filter_type[:date].insert(8, '<<')

          operators_by_filter_type[:date_past].insert(8, '<<')

          operators_by_filter_type[:time_past] = ['>h-', '<h-', '><h-', '<<t', '!*', '*', '><h']

          operators_by_filter_type[:list_multiple] = ['=', '==', '[~]', '!', '!!', '![~]', '!*', '*']
          ...
          def validate_query_filters
            ...
            filters.each_key do |field|
              if values_for(field)
                case type_for(field)
                when :integer
                  add_filter_error(field, :invalid) if values_for(field).detect do |v|
                                                         v.present? && !v.match(/\A[+-]?\d+(\D*?[+-]?\d+)*\z/)
                                                       end
                when :float
                  add_filter_error(field, :invalid) if values_for(field).detect do |v|
                                                         v.present? && !v.match(/\A[+-]?\d+(\.\d*)?\z/)
                                                       end
                when :date, :date_past
              ...
              add_filter_error(field, :blank) unless
                  ...
                  ['o', 'c', '!*', '*', 't', 'ld', 'nd', '<<', '>>', '<<t', 't>>', 'w', 'lw', 'nw', 'l2w', 'm', 'lm',
                   'nm', 'y', '*o', '!o'].include? operator_for(field)
            end
          end # def
      ...
      module InstanceMethods
        def sql_for_field_with_more_filters(field, operator, value, db_table, db_field, is_custom_filter = false)
          if field.include?('status_first_date_') && respond_to?(:sql_for_field_with_dit_kpi)
            sql = sql_for_field_with_dit_kpi(field, operator, value, db_table, db_field, is_custom_filter)
            return sql if sql
          end
          ...
          when '!*'
            ...
            s << " OR RTRIM(#{db_table}.#{db_field}) = ''" if is_custom_filter || %i[text
                                                                                     string].include?(type_for(field))
            s
          when '*'
            s = "#{db_table}.#{db_field} IS NOT NULL"
            s << " AND RTRIM(#{db_table}.#{db_field}) <> ''" if is_custom_filter || %i[text
                                                                                       string].include?(type_for(field))
            s
          when '^='
            sql_begins_with("#{db_table}.#{db_field}", value.first)
          when '*^='
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_begins_with("#{db_table}.#{db_field}", s) }.join(' OR ')
          when '!^='
            sql_begins_with("#{db_table}.#{db_field}", value.first, false)
          when '!*^='
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_begins_with("#{db_table}.#{db_field}", s, false) }.join(' AND ')
          ...
          when '*$='
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_ends_with("#{db_table}.#{db_field}", s) }.join(' OR ')
          when '!$='
            sql_ends_with("#{db_table}.#{db_field}", value.first, false)
          when '!*$='
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_ends_with("#{db_table}.#{db_field}", s, false) }.join(' AND ')

          when '*~'
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_contains("#{db_table}.#{db_field}", s) }.join(' OR ')
          when '!*~'
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_contains("#{db_table}.#{db_field}", s, false) }.join(' AND ')
          when '[~]'
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_contains("#{db_table}.#{db_field}", s) }.join(' AND ')
          when '![~]'
            value.first.split(' ').select do |s|
              s.present?
            end.map { |s| sql_contains("#{db_table}.#{db_field}", s, false) }.join(' OR ')
        ...
        def sql_for_match_operators(_field, operator, value, db_table, db_field, _is_custom_filter = false)
          sql = ''
          v = '(' + value.first.strip + ')'
          ...
          v.chars.each do |c|
            if (!in_bracket && '()+~!'.include?(c) && in_term) || (in_bracket && '}'.include?(c))
              sql += '(' + sql_contains("#{db_table}.#{db_field}", term, match) + ')' unless term.empty?
              # reset
              op = ''
              term = ''
              in_term = false
          ...
          sql = ' NOT ' + sql if operator.include? '!'
        ...
        def sql_for_custom_field_with_more_filters(field, operator, value, custom_field_id)
          ...
          if filter[:field].format.target_class && filter[:field].format.target_class <= User && value.delete('me')
          ...
          if field =~ /^(.+)\.cf_/
            ...
            customized_class = begin
              queried_class.reflect_on_association(assoc.to_sym).klass.base_class
            rescue StandardError
              nil
            end
        ...
        def statement_with_more_filters
          ...
          if filters and valid?
            filters.each_key do |field|
              ...
              if %w[assigned_to_id author_id user_id watcher_id updated_by
                    last_updated_by].include?(field) && v.delete('me')
              ...
              v += User.current.memberships.map(&:project_id).map(&:to_s) if field == 'project_id' && v.delete('mine')

              if field =~ /^cf_(\d+)\.cf_(\d+)$/
                filters_clauses << sql_for_chained_custom_field(field, operator, v, ::Regexp.last_match(1),
                                                                ::Regexp.last_match(2))
              elsif field =~ /cf_(\d+)$/
                # custom field
                if v.is_a?(Array)
                  ...
                  when '=='
                    fc = []
                    v.each do |sv|
                      fc << sql_for_custom_field_with_more_filters(field, '=', [sv], ::Regexp.last_match(1))
                    end
                    filters_clauses << fc.join(' AND ')
                    filters_clauses << (' NOT ' + sql_for_custom_field_with_more_filters(field, '!!', v,
                                                                                         ::Regexp.last_match(1)))
                  when '[~]'
                    fc = []
                    v.each do |sv|
                  ...
                  when '!!'
                    ...
                    filters_clauses << (' NOT (' + fc.join(' AND ') + ' AND NOT ' + sql_for_custom_field_with_more_filters(
                      field, '!!', v, ::Regexp.last_match(1)
                    ) + ')')
                  else
                    filters_clauses << sql_for_custom_field_with_more_filters(field, operator, v,
                                                                              ::Regexp.last_match(1))
                  end
                else
                  filters_clauses << sql_for_custom_field_with_more_filters(field, operator, v, ::Regexp.last_match(1))
                end
              elsif field =~ /^cf_(\d+)\.(.+)$/
                filters_clauses << sql_for_custom_field_attribute(field, operator, v, ::Regexp.last_match(1),
                                                                  ::Regexp.last_match(2))
                ...
                # regular field
                filters_clauses << ('(' + sql_for_field_with_more_filters(field, operator, v, queried_table_name,
                                                                          field) + ')')
              end
            end
          end
        ...
        def sql_one_of(db_field, values, match = true)
          queried_class.send :sanitize_sql_for_conditions,
                             "#{db_field} #{match ? '' : 'NOT'} IN (#{values.map do |v|
        ...
        def relative_time_clause(table, field, from, from_epoch, to, to_epoch)
          ...
          from_epoch ||= 'second'
          to_epoch   ||= 'second'
          s << (format("#{table}.#{field} > %s", Redmine::Database.relative_time(from, from_epoch))) if from
          s << (format("#{table}.#{field} <= %s", Redmine::Database.relative_time(to, to_epoch))) if to
          s.join(' AND ')
        end # def
        ...
        def local_clock_time_clause(table, field, from, to, time_zone)
          ...
          s << (format("(#{Redmine::Database.local_clock_time_sql(table, field, time_zone)}) > '%s'", from)) if from
          s << (format("(#{Redmine::Database.local_clock_time_sql(table, field, time_zone)}) <= '%s'", to)) if to
          s.join(' AND ')
        end # def

        def get_timezone
          return User.current.time_zone_or_default_identifier if User.current
          if RedmineApp::Application.config.time_zone
            return ActiveSupport::TimeZone[RedmineApp::Application.config.time_zone]
          end
...
unless Query.included_modules.include?(RedmineMoreFilters::Patches::QueryPatch)
  Query.include RedmineMoreFilters::Patches::QueryPatch
end
